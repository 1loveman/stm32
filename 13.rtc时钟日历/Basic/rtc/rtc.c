...
void RTC_Config(void){
    if(BKP_ReadBackupRegister(BKP_DR1)==0XA5A5){//
        ...
        RTC_WaitForSynchro();
        RTC_WaitForLastTask();
        RTC_ITConfig(RCC_IT_SEC,DISABLE);
        RTC_WaitForLastTask();
    }
    ...
    if(t_daycnt){
        ...
        while(t_daycnt>=365){//
            if(Is_Leap_Year(t_num)){
                if(t_daycnt>=366)t_daycnt-=(86400*366);
                else braek;
            }
            else counter-=(86400*365);
            t_num++;
        }
        ryear=t_num;
        t_num=1;
        while(t_daycnt>=28){
            if(Is_Leap_Year(ryear&&(t_num==2))){
                if(t_daycnt>=29)counter-=(86400*29);
                else break;
            }
            else{
                if(t_daycnt>=mon_tab[t_num-1])counter-=mon_tab[t_num-1]*(86400);
                else break;
            }
            t_num++
        }
